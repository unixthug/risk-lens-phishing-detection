...
async function loadScore() {
  ...
  if (!entry) {
    ...
    await chrome.runtime.sendMessage({ type: "SCORE_TAB_NOW", tabId: tab.id });
    return;
  }
...
document.getElementById("darkModeBtn").addEventListener("click", async () => {
  ...
  await chrome.storage.local.set({ popupLightMode: isLight });
});
...
document.getElementById("advToggle").addEventListener("click", async () => {
  ...
  await chrome.storage.local.set({ popupAdvancedOpen: open });
});
